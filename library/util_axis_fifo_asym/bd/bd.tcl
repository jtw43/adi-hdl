...
proc init {cellpath otherInfo} {
	...
	bd::mark_propagate_overrideable $ip \
		"S_DATA_WIDTH M_DATA_WIDTH TLAST_EN TKEEP_EN TSTRB_EN TUSER_BITS_PER_BYTE TUSER_WIDTH TID_WIDTH TDEST_WIDTH"
...
proc parameter_set {cellpath} {
	...
	set data_width_slave [get_property "CONFIG.TDATA_NUM_BYTES" $interface_pin_source]
	set_property "CONFIG.S_DATA_WIDTH" [expr $data_width_slave * 8] $ip

	set tuser_width_slave [get_property "CONFIG.TUSER_WIDTH" $interface_pin_source]
	...
	set interface_pin_sink [find_bd_objs -relation connected_to [get_bd_intf_pins -of_objects $ip -filter {NAME =~ m_axis}]]

	set data_width_master [get_property "CONFIG.TDATA_NUM_BYTES" $interface_pin_sink]
	set_property "CONFIG.M_DATA_WIDTH" [expr $data_width_master * 8] $ip

	set tuser_width_master [get_property "CONFIG.TUSER_WIDTH" $interface_pin_sink]

	if {$tuser_width_slave == $tuser_width_master} {
		set_property "CONFIG.TUSER_BITS_PER_BYTE" 0 $ip
		set_property "CONFIG.TUSER_WIDTH" $tuser_width_slave $ip
	} else {
		set ratio_data [expr $data_width_slave / $data_width_master]
		set value [expr log($ratio_data)/log(2)]
		if {$value != floor($value)} {
			error "Ratio is not power of 2!"
		}

		set ratio_user [expr $tuser_width_slave / $tuser_width_master]

		if {$ratio_data != $ratio_user} {
			error "Data width ratio and TUser width ratio are not equal! (TUser Bits per Data Byte)\nOR\nTUser widths are not equal! (TUser per Data Beat)"
		} else {
			set_property "CONFIG.TUSER_BITS_PER_BYTE" 1 $ip
			set_property "CONFIG.TUSER_WIDTH" [expr max($tuser_width_slave, tuser_width_master)] $ip
		}
	}
}
